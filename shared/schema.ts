...
export const harvestLots = pgTable(
  ...
  (t) => ({
    ...
    unitPriceCk: check("harvest_lots_unit_price_ck", sql`${t.unitPrice} IS NULL OR ${t.unitPrice} >= 0`),
    costAllocatedCk: check("harvest_lots_cost_allocated_ck", sql`${t.costAllocated} IS NULL OR ${t.costAllocated} >= 0`),
  }),
);
...
export const userRoleEnum = pgEnum("user_role", ["admin", "tecnico", "encargado", "operario", "finanzas"]);
...
export const attachmentEntityTypeSchema = z.enum([
  "observation", "task", "fieldApplication", "harvestLot", "hiveInspection", "inventoryItem",
]);
...
export const attachmentLocalStatusEnum = pgEnum("attachment_local_status", ["pending", "uploading", "uploaded", "failed"]);
...
export const queenStatusEnum = pgEnum("queen_status", ["seen", "not_seen", "absent", "replaced", "unknown"]);
...
export const adapterReadinessSchema = z.enum([
  "ready",
  "needs-secrets",
  "stub-only",
]);
...
export const importDatasetSchema = z.enum([
  "blocks",
  "greenhouses",
  "inventory",
  "tasks",
...
export const idempotencyKeys = pgTable(
  ...
  (t) => ({
    ...
    stateCk: check(
      ...
      sql`${t.state} in ('processing', 'completed')`,
    ),
  }),
);
...
export const insertFieldApplicationSchema = fieldApplicationSchema.omit({
  id: true, scopeName: true, safeHarvestDate: true, movementId: true, createdAt: true,
});
export type InsertFieldApplication = z.infer<typeof insertFieldApplicationSchema>;

/* ----- Beekeeping zod ----- */
export const apiarySchema = z.object({
  id: z.string(), name: z.string().min(1), location: z.string().min(1),
  lat: z.number().optional(), lng: z.number().optional(),
  notes: z.string().optional(), status: operationalStatusSchema, createdAt: z.string(),
});
export type Apiary = z.infer<typeof apiarySchema>;
export const insertApiarySchema = apiarySchema.omit({ id: true, createdAt: true })
...
export const hiveSchema = z.object({
  id: z.string(), apiaryId: z.string(), code: z.string().min(1),
  status: operationalStatusSchema,
  queenStatus: queenStatusSchema, colonyStrength: colonyLevelSchema,
  broodLevel: broodLevelSchema, honeyStores: honeyLevelSchema,
  lastInspectionAt: z.string().optional(), notes: z.string().optional(),
  createdAt: z.string(),
});
export type Hive = z.infer<typeof hiveSchema>;
export const insertHiveSchema = hiveSchema.omit({ id: true, lastInspectionAt: true, createdAt: true })
  .extend({
...
export const hiveInspectionSchema = z.object({
  id: z.string(), hiveId: z.string(), inspectedAt: z.string(), inspector: z.string().min(1),
  queenSeen: z.boolean(), queenStatus: queenStatusSchema,
  colonyStrength: colonyLevelSchema, broodLevel: broodLevelSchema, honeyStores: honeyLevelSchema,
  pestsOrDisease: z.string().optional(), feedingGiven: z.string().optional(), treatmentGiven: z.string().optional(),
  inventoryItemId: z.string().optional(), quantityUsed: z.number().nonnegative().optional(),
  movementId: z.string().optional(), notes: z.string().optional(),
  hasPhotos: z.number().int().nonnegative(), createdAt: z.string(),
});
...
export const insertLaborCostSchema = laborCostSchema.omit({ id: true, createdAt: true, expenseId: true })
  .extend({ currency: z.string().min(1).max(8).default("BOB") });
export type InsertLaborCost = z.infer<typeof insertLaborCostSchema>;

export const honeyHarvestSchema = z.object({
  id: z.string(), apiaryId: z.string(), hiveId: z.string().optional(),
  date: z.string(), quantity: z.number().positive(), unit: z.string().min(1),
  destination: z.string().optional(), notes: z.string().optional(), createdAt: z.string(),
});
