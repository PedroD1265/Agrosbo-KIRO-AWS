...
import {
  queueCreateGreenhouse,
  queueUpdateGreenhouse,
...
export function GreenhouseForm({ greenhouse, onDone }: Props) {
  ...
  const submit = form.handleSubmit(async (values) => {
    const payload = {
      ...
      humidity: values.humidity === "" || values.humidity === undefined ? undefined : Number(values.humidity),
    };
    try {
      if (isEdit && greenhouse) {
        ...
        await queueCreateGreenhouse(payload as unknown as Parameters<typeof queueCreateGreenhouse>[0]);
        toast.success("Invernadero creado", {
          description: navigator.onLine ? "Sincronizando…" : "Encolado offline. Se enviará al recuperar internet.",
        });
      }
      form.reset();
      onDone();
      ...
      <form className="space-y-6" onSubmit={submit}>
        <FormSection title="Identificación" description="Cómo lo reconoces en campo">
          <FormField control={form.control} name="name" render={({ field }) => (
            <FormItem><FormLabel>Nombre del invernadero</FormLabel><FormControl><Input placeholder="Ej: Invernadero N1" autoFocus {...field} data-testid="input-gh-name" /></FormControl><FormMessage /></FormItem>
          )} />
          <div className="grid grid-cols-2 gap-3">
            <FormField control={form.control} name="areaM2" render={({ field }) => (
              <FormItem><FormLabel>Superficie (m²)</FormLabel><FormControl><Input type="number" inputMode="numeric" step="1" {...field} data-testid="input-gh-area" /></FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="crop" render={({ field }) => (
              <FormItem><FormLabel>Cultivo</FormLabel><FormControl><Input placeholder="Ej: Tomate" {...field} data-testid="input-gh-crop" /></FormControl><FormMessage /></FormItem>
            )} />
          </div>
          <FormField control={form.control} name="variety" render={({ field }) => (
            <FormItem><FormLabel>Variedad <span className="text-muted-foreground">(opcional)</span></FormLabel><FormControl><Input placeholder="Ej: Cherry" {...field} data-testid="input-gh-variety" /></FormControl><FormMessage /></FormItem>
          )} />
        </FormSection>
        ...
        <FormSection title="Estado" description="Etapa del cultivo y estado operativo">
          <div className="grid grid-cols-2 gap-3">
            <FormField control={form.control} name="stage" render={({ field }) => (
              <FormItem><FormLabel>Etapa</FormLabel>
                <Select value={field.value} onValueChange={field.onChange}>
                  <FormControl><SelectTrigger data-testid="select-gh-stage"><SelectValue /></SelectTrigger></FormControl>
                  <SelectContent>
            ...
            <FormField control={form.control} name="status" render={({ field }) => (
              <FormItem><FormLabel>Estado operativo</FormLabel>
                <Select value={field.value} onValueChange={field.onChange}>
                  <FormControl><SelectTrigger data-testid="select-gh-status"><SelectValue /></SelectTrigger></FormControl>
                  <SelectContent>
        ...
        <FormSection title="Condiciones ambientales" description="Lecturas actuales (opcional)">
          <div className="grid grid-cols-2 gap-3">
            <FormField control={form.control} name="tempC" render={({ field }) => (
              <FormItem><FormLabel>Temperatura (°C)</FormLabel><FormControl><Input type="number" inputMode="decimal" step="0.1" placeholder="—" {...field} data-testid="input-gh-temp" /></FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="humidity" render={({ field }) => (
              <FormItem><FormLabel>Humedad (%)</FormLabel><FormControl><Input type="number" inputMode="numeric" step="1" placeholder="—" {...field} data-testid="input-gh-humidity" /></FormControl><FormMessage /></FormItem>
            )} />
          </div>
        </FormSection>

        <FormFooter offlineAware={!isEdit}>
          <Button type="submit" className="w-full" size="lg" disabled={submitting} data-testid="button-submit-gh">
