      ...
      <form className="space-y-6" onSubmit={submit}>
        <FormSection title="Identificación" description="Cómo lo reconoces en campo">
          <FormField control={form.control} name="name" render={({ field }) => (
            <FormItem><FormLabel>Nombre del bloque</FormLabel><FormControl><Input placeholder="Ej: Bloque A1" autoFocus {...field} data-testid="input-block-name" /></FormControl><FormMessage /></FormItem>
          )} />
          <div className="grid grid-cols-2 gap-3">
            <FormField control={form.control} name="farm" render={({ field }) => (
              <FormItem><FormLabel>Predio</FormLabel><FormControl><Input placeholder="Ej: Predio Norte" {...field} data-testid="input-block-farm" /></FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="areaHa" render={({ field }) => (
              <FormItem><FormLabel>Superficie (ha)</FormLabel><FormControl><Input type="number" inputMode="decimal" step="0.01" {...field} data-testid="input-block-area" /></FormControl><FormMessage /></FormItem>
            )} />
          </div>
        </FormSection>
        ...
        <FormSection title="Cultivo" description="Qué se está produciendo">
          <div className="grid grid-cols-2 gap-3">
            <FormField control={form.control} name="crop" render={({ field }) => (
              <FormItem><FormLabel>Cultivo</FormLabel><FormControl><Input placeholder="Ej: Soya" {...field} data-testid="input-block-crop" /></FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="variety" render={({ field }) => (
              <FormItem><FormLabel>Variedad <span className="text-muted-foreground">(opcional)</span></FormLabel><FormControl><Input placeholder="Ej: Munasqa" {...field} data-testid="input-block-variety" /></FormControl><FormMessage /></FormItem>
            )} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <FormField control={form.control} name="stage" render={({ field }) => (
              <FormItem><FormLabel>Etapa</FormLabel>
                <Select value={field.value} onValueChange={field.onChange}>
                  <FormControl><SelectTrigger data-testid="select-block-stage"><SelectValue /></SelectTrigger></FormControl>
                  <SelectContent>
            ...
            <FormField control={form.control} name="status" render={({ field }) => (
              <FormItem><FormLabel>Estado operativo</FormLabel>
                <Select value={field.value} onValueChange={field.onChange}>
                  <FormControl><SelectTrigger data-testid="select-block-status"><SelectValue /></SelectTrigger></FormControl>
                  <SelectContent>
        ...
        <FormSection title="Riego" description="Última actualización conocida">
          <FormField control={form.control} name="lastIrrigation" render={({ field }) => (
            <FormItem><FormLabel>Último riego</FormLabel><FormControl><Input type="date" {...field} data-testid="input-block-irrigation" /></FormControl><FormMessage /></FormItem>
          )} />
        </FormSection>

        <FormFooter offlineAware={!isEdit}>
          <Button type="submit" className="w-full" size="lg" disabled={submitting} data-testid="button-submit-block">
