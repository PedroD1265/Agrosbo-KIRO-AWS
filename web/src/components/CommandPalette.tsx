    ...
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      ...
      <CommandList>
        ...
        <CommandGroup heading="Acciones rápidas">
          ...
          <CommandItem value="nuevo riego" onSelect={() => go('/irrigation')}>
            <Plus className="h-4 w-4" />
            <span>Nuevo riego</span>
            <CommandShortcut>n r</CommandShortcut>
          </CommandItem>
          <CommandItem value="nueva observacion" onSelect={() => go('/observations')}>
            <Plus className="h-4 w-4" />
            ...
            <CommandGroup heading="Bloques">
              {blocks.slice(0, 20).map((b) => (
                ...
                >
                  ...
                  <span>{b.name}</span>
                  {b.crop && (
                    <span className="ml-auto text-xs text-muted-foreground">{b.crop}</span>
                  )}
                </CommandItem>
              ))}
            </CommandGroup>
            ...
            <CommandGroup heading="Invernaderos">
              {greenhouses.slice(0, 20).map((g) => (
                ...
                >
                  ...
                  <span>{g.name}</span>
                  {g.crop && (
                    <span className="ml-auto text-xs text-muted-foreground">{g.crop}</span>
                  )}
                </CommandItem>
              ))}
            </CommandGroup>
