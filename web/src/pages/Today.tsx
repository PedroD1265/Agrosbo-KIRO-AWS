...
import {
  Droplets, ListChecks, AlertTriangle, NotebookPen, ChevronRight,
  Plus, Sprout, LayoutGrid, Sun, MapPin, ClipboardList, Activity, CalendarClock,
  CalendarRange, Zap,
} from "lucide-react";
...
import {
  useAlerts, useBlocks, useGreenhouses, useIrrigationEvents, useObservations, useTasks, useCampaigns,
} from "@/hooks/data";
import {
  rankTask, rankIrrigation, rankBlockRisk, getDueUrgency, getIrrigationUrgency, alertsBySeverity,
...
export default function TodayPage() {
  ...
  const overdueIrr = sortedIrrigation.filter((i) => getIrrigationUrgency(i.scheduledAt) === "overdue");
  ...
  const attentionCount = overdueTasks.length + todayTasks.length + overdueIrr.length + todayIrr.length;
  ...
  const farmCentroid = useMemo(() => {
    const withCoords = blocks
      .map((b) => ({ lat: (b as any).centroidLat as number | null, lng: (b as any).centroidLng as number | null }))
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-hero p-8 text-primary-foreground shadow-elevated md:p-10">
          <div className="relative z-10 max-w-2xl">
            ...
            <p className="mt-3 text-sm text-primary-foreground/85 md:text-base">
              Registra al menos un bloque o invernadero para empezar a programar riegos, asignar tareas,
              registrar cosechas y observaciones de campo.
            </p>
            <div className="mt-5 flex flex-wrap gap-2">
              <Button size="lg" variant="secondary" asChild data-testid="button-onboard-block">
                <Link to="/blocks"><LayoutGrid className="h-4 w-4" /> Crear bloque</Link>
              </Button>
              <Button
                size="lg"
                asChild
              ...
              >
                <Link to="/greenhouses"><Sprout className="h-4 w-4" /> Crear invernadero</Link>
              </Button>
            </div>
          </div>
        ...
        <Card className="shadow-card">
          <CardHeader className="pb-3"><CardTitle className="text-base">Próximos pasos sugeridos</CardTitle></CardHeader>
          <CardContent>
  ...
  const heroTitle = overdueTotal > 0
    ...
    <div className="space-y-5 animate-fade-in pb-4">
        ...
        <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-primary-foreground/80 md:text-sm">
              <span className="inline-flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" /> Toco, Cochabamba</span>
          ...
          <div className="flex shrink-0 items-stretch gap-2">
            <div className={cn(
              ...
              overdueTotal > 0 ? "bg-status-critical/25 ring-1 ring-status-critical/40" : "bg-primary-foreground/10",
            )}>
              <span className="text-2xl font-semibold tabular leading-none md:text-[28px]">{overdueTotal}</span>
              <span className="mt-1 text-[10px] uppercase tracking-wider text-primary-foreground/85">vencidos</span>
            </div>
            <div className="flex flex-col justify-center rounded-xl border border-primary-foreground/15 bg-primary-foreground/10 px-3.5 py-2 backdrop-blur-sm min-w-[68px]">
              <span className="text-2xl font-semibold tabular leading-none md:text-[28px]">{todayTasks.length + todayIrr.length}</span>
              <span className="mt-1 text-[10px] uppercase tracking-wider text-primary-foreground/85">de hoy</span>
            </div>
            <div className="hidden flex-col justify-center rounded-xl border border-primary-foreground/15 bg-primary-foreground/10 px-3.5 py-2 backdrop-blur-sm min-w-[68px] md:flex">
              <span className="text-2xl font-semibold tabular leading-none md:text-[28px]">{completedToday}</span>
              <span className="mt-1 text-[10px] uppercase tracking-wider text-primary-foreground/85">hechas</span>
            </div>
          </div>
        </div>
      ...
      {dayStable && (
        <div className="flex items-center gap-3 rounded-xl border border-status-ok/30 bg-status-ok-soft/50 px-4 py-3 text-sm text-status-ok">
          ...
          <p className="text-status-ok/80">Sin vencidos, sin alertas críticas, sin unidades en riesgo.</p>
        </div>
      )}
      ...
      <OnboardingHints
        steps={[
          { id: "unit", label: "Crear bloque o invernadero", to: "/blocks", done: blocks.length + greenhouses.length > 0, icon: LayoutGrid },
          { id: "campaign", label: "Iniciar primera campaña", to: "/campaigns", done: campaigns.length > 0, icon: CalendarRange },
          { id: "task", label: "Asignar primera tarea", to: "/tasks", done: tasks.length > 0, icon: ListChecks },
          { id: "irrigation", label: "Programar primer riego", to: "/irrigation", done: irrigation.length > 0, icon: Droplets },
        ]}
      />
        ...
        <div className="space-y-5">
            ...
            <Card className="shadow-card">
              <CardHeader className="flex-row items-center justify-between space-y-0 pb-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">Vencidas y de hoy, ordenadas por urgencia</p>
                </div>
                <Button variant="ghost" size="sm" asChild>
                  <Link to="/tasks">Todas <ChevronRight className="h-4 w-4" /></Link>
                </Button>
              </CardHeader>
              <CardContent className="space-y-2">
                ...
                <Button variant="outline" size="sm" className="w-full" asChild>
                  <Link to="/tasks"><Plus className="h-4 w-4" /> {sortedTasks.length === 0 ? "Asignar nueva tarea" : "Crear nueva tarea"}</Link>
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-card">
              <CardHeader className="flex-row items-center justify-between space-y-0 pb-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">Programados, vencidos o en próximas horas</p>
                </div>
                <Button variant="ghost" size="sm" asChild>
                  <Link to="/irrigation">Todos <ChevronRight className="h-4 w-4" /></Link>
                </Button>
              </CardHeader>
              <CardContent className="space-y-2">
                {sortedIrrigation.length === 0 ? (
                ...
                <Button variant="outline" size="sm" className="w-full" asChild>
                  <Link to="/irrigation"><Plus className="h-4 w-4" /> {sortedIrrigation.length === 0 ? "Programar primer riego" : "Programar riego"}</Link>
                </Button>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="shadow-card">
            <CardHeader className="flex-row items-center justify-between space-y-0 pb-3">
              ...
              <Button variant="ghost" size="sm" asChild>
                <Link to="/blocks">Ver bloques <ChevronRight className="h-4 w-4" /></Link>
              </Button>
            </CardHeader>
            <CardContent>
              {riskEntities.length === 0 ? (
                ...
                <div className="grid gap-2.5 md:grid-cols-2">
                  {riskEntities.slice(0, 6).map((e) => (
                    <EntityRiskCard
                      key={`${e.kind}-${e.id}`}
                      entity={e}
                      lastIrrigation={e.kind === "block" ? new Date(e.lastIrrigation).toLocaleDateString("es-BO", { day: "2-digit", month: "short" }) : undefined}
                    />
                  ))}
                </div>
              )}
        ...
        <aside className="space-y-5 xl:sticky xl:top-4 xl:self-start">
          <Card className="shadow-card">
            <CardHeader className="pb-3">
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">Captura desde campo en segundos</p>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-2 xl:grid-cols-1">
                <QuickActionTile to="/irrigation" icon={Droplets} label="Registrar riego" hint="Programar o ejecutar" tone="primary" testId="qa-irrigation" />
                <QuickActionTile to="/tasks" icon={ListChecks} label="Crear tarea" hint="Asignar al equipo" tone="accent" testId="qa-task" />
                <QuickActionTile to="/observations" icon={NotebookPen} label="Observación" hint="Bitácora de campo" tone="default" testId="qa-obs" />
                <QuickActionTile to="/harvest" icon={ClipboardList} label="Registrar cosecha" hint="Crear lote" tone="warn" testId="qa-harvest" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-card">
            ...
            <CardContent>
              {observations.length === 0 ? (
                ...
                <Timeline
                  items={(recentIncidents.length > 0 ? recentIncidents : observations.slice(0, 5)).map((o) => ({
                    id: o.id,
                    title: `${o.scopeName} · ${o.author}`,
                    description: o.text,
                    meta: new Date(o.createdAt).toLocaleString("es-BO", { dateStyle: "short", timeStyle: "short" }),
                    tone:
