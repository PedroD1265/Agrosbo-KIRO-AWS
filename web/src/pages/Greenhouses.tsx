...
export default function GreenhousesPage() {
  ...
  const filtered = greenhouses.filter((g) =>
    [g.name, g.crop].join(' ').toLowerCase().includes(q.toLowerCase()),
  );
    ...
    <div className="space-y-5">
      <PageHeader
        ...
        actions={
          <QuickCaptureDrawer
            ...
            trigger={
              <Button size="sm" data-testid="button-new-gh">
      ...
      {greenhouses.length > 0 && (
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <MetricCard
            ...
            hint={alertCount === 0 ? 'Sin alertas' : 'Revisar pronto'}
          />
        </div>
      )}
      <FilterBar
        value={q}
        onValueChange={setQ}
        placeholder="Buscar invernadero o cultivo…"
        resultCount={greenhouses.length > 0 ? filtered.length : undefined}
      />
      ...
      {greenhouses.length === 0 ? (
        <EmptyState
          ...
          action={
            <Button size="sm" onClick={() => setOpenCreate(true)} data-testid="button-empty-new-gh">
              <Plus className="h-4 w-4" /> Crear invernadero
            </Button>
          }
        />
      ) : (
