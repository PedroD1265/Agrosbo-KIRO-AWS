          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select value={field.value} onValueChange={field.onChange}>
                <FormControl>
                  <SelectTrigger data-testid="select-task-consume-item">
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  step="0.01"
                  {...field}
                  data-testid="input-task-consume-amount"
                />
              </FormControl>
              <FormMessage />
            </FormItem>
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  type="number"
                  step="0.01"
                  placeholder={selectedItem?.unitCost?.toString() ?? '0.00'}
                  {...field}
                  value={field.value ?? ''}
                  data-testid="input-task-consume-cost"
                />
              </FormControl>
            </FormItem>
          )}
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  {...field}
                  data-testid="input-task-consume-note"
                />
              </FormControl>
            </FormItem>
          )}
  ...
  const filtered = useMemo(() => {
    ...
    return [...base].sort((a, b) => {
      switch (sortKey) {
        ...
        case 'scope':
          return sign * a.scopeName.localeCompare(b.scopeName);
        case 'assignee':
          return sign * (a.assignee ?? '').localeCompare(b.assignee ?? '');
        case 'due':
    ...
    <div className={`space-y-5 animate-fade-in ${isMobile ? 'pb-24' : ''}`}>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <MetricCard
          ...
          icon={AlertTriangle}
          tone={counts.overdue > 0 ? 'critical' : 'ok'}
          hint={counts.overdue === 0 ? 'Sin vencidas' : 'Atender pronto'}
        />
      ...
      {isMobile ? (
        <div className="space-y-4">
          ...
          <MobileActionBar
            hint={counts.overdue > 0 ? `${counts.overdue} vencidas requieren atención` : undefined}
          >
            <Button
              size="lg"
                    ...
                    footer={
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          data-testid="button-task-edit-panel"
                        >
                          Editar
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => setConsumeTask(selectedTask)}
                          data-testid="button-task-consume-panel"
                        >
                          Consumir insumo
                        </Button>
                      </div>
                  ...
                  >
                    <dl className="grid grid-cols-2 gap-3 text-[13px]">
                      <div>
                        ...
                        <dd
                          className={`tabular ${isOverdue(selectedTask) ? 'font-semibold text-status-critical' : ''}`}
                        >
                          {selectedTask.dueDate}
                          {isOverdue(selectedTask) && (
                            <span className="ml-1 text-[10px] uppercase">vencida</span>
                          )}
                        </dd>
                      </div>
                      <div>
                      ...
                      <div className="col-span-2">
                        ...
                        <dd>
                          ...
                          <span className="text-muted-foreground">
                            ({selectedTask.scopeType === 'block' ? 'Bloque' : 'Invernadero'})
                          </span>
                        </dd>
                      </div>
                      <div className="col-span-2">
