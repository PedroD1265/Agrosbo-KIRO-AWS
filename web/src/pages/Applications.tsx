    ...
    <Form {...form}>
      <form className="space-y-4" onSubmit={submit}>
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select value={field.value} onValueChange={field.onChange}>
                <FormControl>
                  <SelectTrigger data-testid="select-app-scope">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} data-testid="input-app-product" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Insumo del inventario (opcional, descuenta stock)</FormLabel>
              <Select value={field.value ?? ''} onValueChange={field.onChange}>
                <FormControl>
                  <SelectTrigger>
                    <SelectValue placeholder="Sin descuento de inventario" />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
                  {inventory.map((i) => (
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Unidad dosis</FormLabel>
                <FormControl>
                  <Input placeholder="ml/L" {...field} value={field.value ?? ''} />
                </FormControl>
              </FormItem>
            )}
          />
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Objetivo / problema</FormLabel>
                <FormControl>
                  <Input placeholder="Tizón, mosca, etc." {...field} value={field.value ?? ''} />
                </FormControl>
              </FormItem>
            )}
          />
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Carencia (días)</FormLabel>
                <FormControl>
                  <Input
                    type="number"
                    min={0}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Método (opcional)</FormLabel>
              <FormControl>
                <Input placeholder="Foliar / drench" {...field} value={field.value ?? ''} />
              </FormControl>
            </FormItem>
          )}
        />
        ...
        />
        <Button type="submit" className="w-full" data-testid="button-submit-app">
    ...
    <div className="space-y-5 animate-fade-in">
        ...
        actions={
          canWrite && (
            <QuickCaptureDrawer
              ...
              trigger={
                <Button size="sm" data-testid="button-new-app">
      ...
      {applications.length === 0 ? (
        <EmptyState
          ...
          description="Registra fertilizantes, fungicidas u otros tratamientos."
        />
      ) : (
            ...
            return (
              <Card key={a.id} data-testid={`card-app-${a.id}`}>
                <CardContent className="p-5 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {a.scopeName} · {fmtDateTime(a.appliedAt)} · {a.responsible}
                      </p>
                    </div>
                    <Badge variant="outline">
                      {TYPE_LABEL[a.applicationType] ?? a.applicationType}
                    </Badge>
                  </div>
                  <p className="text-sm">
                  ...
                  {a.targetProblem && (
                    <p className="text-xs text-muted-foreground">Objetivo: {a.targetProblem}</p>
                  )}
                  {carencia && (
                  ...
                  {a.movementId && (
                    <p className="text-[10px] text-muted-foreground">
                      Inventario descontado · mov {a.movementId}
                    </p>
                  )}
                </CardContent>
              </Card>
