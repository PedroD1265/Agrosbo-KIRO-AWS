...
const CSV_REPORTS: { key: string; label: string; description: string }[] = [
  { key: "applications", label: "Aplicaciones fitosanitarias", description: "Productos, dosis, áreas tratadas" },
...
const REPORTS: ReportItem[] = [
  { key: "resumen", label: "Resumen operativo", description: "KPIs globales del campo", icon: LayoutDashboard, group: "Visión general" },
  { key: "bloques", label: "Por bloque", description: "Estado consolidado de parcelas", icon: LayoutGrid, group: "Por entidad" },
  { key: "invernaderos", label: "Por invernadero", description: "Estado de entornos protegidos", icon: Warehouse, group: "Por entidad" },
  { key: "riego", label: "Cumplimiento riego", description: "Programado vs realizado", icon: Droplets, group: "Operación" },
  { key: "tareas", label: "Tareas", description: "Vencidas, pendientes y ejecución", icon: CheckSquare, group: "Operación" },
  { key: "inventario", label: "Inventario y cosecha", description: "Stock crítico y lotes producidos", icon: Package, group: "Operación" },
  { key: "observaciones", label: "Observaciones", description: "Bitácora de campo e incidencias", icon: Eye, group: "Operación" },
];
      ...
      <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
        ...
        <section className="min-w-0 space-y-4">
          ...
          <div className="flex items-start gap-3 rounded-xl border border-border/60 bg-gradient-to-br from-primary-soft/40 to-transparent px-4 py-3">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-primary/80">{current.group}</p>
            ...
            <CardContent className="p-4">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Descarga el dataset completo en formato CSV</p>
                </div>
              </div>
                ...
                {CSV_REPORTS.map((r) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-[10px] leading-tight text-muted-foreground">{r.description}</p>
                    </div>
                  </a>
                ))}
              </div>
          ...
          <div data-testid={`report-${active}`}>
            ...
            {active === "riego" && <CumplimientoRiego filters={irrigationFilters} onFiltersChange={setIrrigationFilters} />}
            {active === "tareas" && <ReporteTareas filters={taskFilters} onFiltersChange={setTaskFilters} />}
            {active === "inventario" && <InventarioCosecha filters={inventoryFilters} onFiltersChange={setInventoryFilters} />}
            {active === "observaciones" && <ObservacionesExport filters={observationFilters} onFiltersChange={setObservationFilters} />}
          </div>
        </section>
      </div>
    </div>
