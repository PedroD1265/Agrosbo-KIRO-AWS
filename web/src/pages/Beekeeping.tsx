  ...
  return (
    <Form {...form}>
      <form className="space-y-3" onSubmit={submit}>
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} data-testid="input-apiary-name" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        />
        <Button type="submit" className="w-full" data-testid="button-submit-apiary">
          Crear apiario
        </Button>
      </form>
    </Form>
...
function NewHiveForm({ onDone }: { onDone: () => void }) {
  ...
  const submit = form.handleSubmit(async (v) => {
    try {
      await queueCreateHive({ apiaryId: v.apiaryId, code: v.code, notes: v.notes });
    ...
    <Form {...form}>
      <form className="space-y-3" onSubmit={submit}>
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Apiario</FormLabel>
              <Select value={field.value} onValueChange={field.onChange}>
                <FormControl>
                  <SelectTrigger data-testid="select-hive-apiary">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} placeholder="C-001" data-testid="input-hive-code" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        />
        <Button type="submit" className="w-full" data-testid="button-submit-hive">
          Crear colmena
        </Button>
      </form>
    </Form>
  ...
  const submit = form.handleSubmit(async (v) => {
    try {
      await queueCreateInspection({
        ...
        queenStatus: v.queenStatus,
        colonyStrength: v.colonyStrength,
        broodLevel: v.broodLevel,
        honeyStores: v.honeyStores,
      ...
      <form className="space-y-3" onSubmit={submit}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Reservas miel</FormLabel>
                <Select value={field.value} onValueChange={field.onChange}>
                  <FormControl>
                    <SelectTrigger>
                      <SelectValue />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Plagas/enfermedades</FormLabel>
              <FormControl>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Jarabe 1:1" {...field} value={field.value ?? ''} />
                </FormControl>
              </FormItem>
            )}
          />
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Ácido oxálico" {...field} value={field.value ?? ''} />
                </FormControl>
              </FormItem>
            )}
          />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Insumo (opcional, descuenta stock)</FormLabel>
              <Select value={field.value ?? ''} onValueChange={field.onChange}>
                <FormControl>
                  <SelectTrigger>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Cant. usada</FormLabel>
              <FormControl>
        ...
        />
        <Button type="submit" className="w-full" data-testid="button-submit-inspection">
...
function NewHoneyForm({ onDone }: { onDone: () => void }) {
  ...
  const submit = form.handleSubmit(async (v) => {
    try {
      ...
      toast.success('Cosecha de miel registrada');
      form.reset();
      onDone();
    } catch (err) {
      toast.error('Error', { description: (err as Error).message });
    ...
    <Form {...form}>
      <form className="space-y-3" onSubmit={submit}>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Colmena (opcional)</FormLabel>
              <Select value={field.value ?? ''} onValueChange={field.onChange}>
                <FormControl>
                  <SelectTrigger>
                    <SelectValue placeholder="Cosecha por apiario" />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
                  {hives
                    .filter((h) => h.apiaryId === watchApiary)
                    .map((h) => (
                      <SelectItem key={h.id} value={h.id}>
                        {h.code}
                      </SelectItem>
        ...
        />
        <Button type="submit" className="w-full" data-testid="button-submit-honey">
          Registrar cosecha
        </Button>
      </form>
    </Form>
        ...
        actions={
          canWrite ? (
            <div className="flex gap-2 flex-wrap">
              <QuickCaptureDrawer
                open={openApiary}
                onOpenChange={setOpenApiary}
                trigger={
                  <Button size="sm" variant="outline" data-testid="button-new-apiary">
                    <Plus className="h-4 w-4" /> Apiario
                  </Button>
                }
                title="Nuevo apiario"
              >
                <NewApiaryForm onDone={() => setOpenApiary(false)} />
              </QuickCaptureDrawer>
              <QuickCaptureDrawer
                open={openHive}
                onOpenChange={setOpenHive}
                trigger={
                  <Button size="sm" variant="outline" data-testid="button-new-hive">
                    <Plus className="h-4 w-4" /> Colmena
                  </Button>
                }
                title="Nueva colmena"
              >
                <NewHiveForm onDone={() => setOpenHive(false)} />
              </QuickCaptureDrawer>
              <QuickCaptureDrawer
                open={openInsp}
                onOpenChange={setOpenInsp}
                trigger={
                  <Button size="sm" data-testid="button-new-inspection">
                    <Plus className="h-4 w-4" /> Inspección
                  </Button>
                }
                title="Inspección de colmena"
              >
                <NewInspectionForm onDone={() => setOpenInsp(false)} />
              </QuickCaptureDrawer>
              <QuickCaptureDrawer
                open={openHoney}
                onOpenChange={setOpenHoney}
                trigger={
                  <Button size="sm" variant="outline" data-testid="button-new-honey">
                    <Plus className="h-4 w-4" /> Miel
                  </Button>
                }
                title="Cosecha de miel"
              >
                <NewHoneyForm onDone={() => setOpenHoney(false)} />
              </QuickCaptureDrawer>
            </div>
          ) : undefined
        }
      ...
      <Tabs defaultValue="hives">
        ...
        <TabsContent value="hives" className="mt-4">
          {hives.length === 0 ? (
            <EmptyState
              title="Sin colmenas"
              description="Crea un apiario y luego añade colmenas."
            />
          ) : (
            <div className="grid gap-3 md:grid-cols-2">
              {hives.map((h) => {
                const apiary = apiaries.find((a) => a.id === h.apiaryId);
                const days = h.lastInspectionAt
                  ? Math.floor((Date.now() - Date.parse(h.lastInspectionAt)) / 86_400_000)
                  : null;
                return (
                  <Card key={h.id} data-testid={`card-hive-${h.id}`}>
                    <CardContent className="p-5 space-y-2">
                      <div className="flex justify-between items-start">
                        ...
                        {h.queenStatus === 'absent' && (
                          <Badge variant="destructive">Sin reina</Badge>
                        )}
                        {h.colonyStrength === 'weak' && (
                          <Badge className="bg-status-warn/20 text-status-warn">Débil</Badge>
                        )}
                      </div>
        ...
        <TabsContent value="apiaries" className="mt-4">
          {apiaries.length === 0 ? (
            <EmptyState title="Sin apiarios" description="Crea un apiario para empezar." />
          ) : (
            <div className="grid gap-3 md:grid-cols-2">
              {apiaries.map((a) => (
                <Card key={a.id} data-testid={`card-apiary-${a.id}`}>
                  <CardContent className="p-5 space-y-1">
                    ...
                    <p className="text-xs">
                      {hives.filter((h) => h.apiaryId === a.id).length} colmenas
                    </p>
                  </CardContent>
                </Card>
              ))}
                    ...
                    <CardContent className="p-4 space-y-1">
                      <div className="flex justify-between">
                        <p className="font-semibold">
                          {hive?.code ?? i.hiveId} ·{' '}
                          {new Date(i.inspectedAt).toLocaleDateString('es-BO')}
                        </p>
                        <p className="text-xs text-muted-foreground">{i.inspector}</p>
                      </div>
                      <p className="text-sm">
                        Reina: {QUEEN_LABEL[i.queenStatus]} · Colonia: {i.colonyStrength} · Cría:{' '}
                        {i.broodLevel} · Miel: {i.honeyStores}
                      </p>
                      {i.pestsOrDisease && (
                        <p className="text-sm text-status-warn">⚠ {i.pestsOrDisease}</p>
                      )}
                      {i.treatmentGiven && (
                        <p className="text-xs">Tratamiento: {i.treatmentGiven}</p>
                      )}
                      {i.movementId && (
                        <p className="text-[10px] text-muted-foreground">
                          Inventario descontado · {i.movementId}
                        </p>
                      )}
                    </CardContent>
                  </Card>
        ...
        <TabsContent value="honey" className="mt-4">
          {honey.length === 0 ? (
            <EmptyState title="Sin cosechas de miel" />
          ) : (
            <div className="space-y-2">
              {honey.map((h) => (
                <Card key={h.id}>
                  <CardContent className="p-4 flex justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {apiaries.find((a) => a.id === h.apiaryId)?.name} · {h.date}
                      </p>
                    </div>
