...
const LAYER_META: Record<LayerKey, { label: string; icon: typeof Square; swatch: string }> = {
  ...
  observations: {
    label: 'Observaciones',
    icon: MapPin,
    swatch: 'bg-status-warn/40 border-status-warn',
  },
};
...
function EntityList({
  ...
  const filteredBlocks = blocks.filter(
    (b) => !q || b.name.toLowerCase().includes(q) || b.farm.toLowerCase().includes(q),
  );
  const filteredGh = greenhouses.filter(
    (g) => !q || g.name.toLowerCase().includes(q) || g.crop.toLowerCase().includes(q),
  );
                  ...
                  return (
                    <li key={b.id}>
                      ...
                      >
                        <div
                          className={cn(
                            'mt-1 h-2 w-2 shrink-0 rounded-sm border',
                            LAYER_META.blocks.swatch,
                          )}
                        />
                        <div className="min-w-0 flex-1">
        ...
        {!showBlocks && !showGh && (
          <div className="p-6">
            <EmptyState
              icon={EyeOff}
              title="Capas ocultas"
              description="Activa al menos una capa para ver entidades."
            />
          </div>
        )}
      </div>
...
export default function MapPage() {
  ...
  const blockCoverage =
    counts.blocksTotal > 0 ? Math.round((counts.blocksMapped / counts.blocksTotal) * 100) : 0;
  const ghCoverage = counts.ghTotal > 0 ? Math.round((counts.ghMapped / counts.ghTotal) * 100) : 0;
  const obsCoverage =
    counts.obsTotal > 0 ? Math.round((counts.obsLocated / counts.obsTotal) * 100) : 0;
  ...
  const openSelected = () => {
    ...
    else if (selected.kind === 'greenhouse' || selected.kind === 'greenhouse-point')
      navigate(`/greenhouses/${selected.id}`);
  ...
  const selectedBlock =
    selected?.kind === 'block' ? blocks.find((b) => b.id === selected.id) : undefined;
  const selectedGh =
    selected && (selected.kind === 'greenhouse' || selected.kind === 'greenhouse-point')
      ? greenhouses.find((g) => g.id === selected.id)
      : undefined;
  const selectedObs =
    selected?.kind === 'observation' ? observations.find((o) => o.id === selected.id) : undefined;
  ...
  return (
    <div className="space-y-4">
        ...
        meta={
          <>
            <Badge
              ...
              data-testid="badge-blocks-coverage"
            >
            ...
            <Badge
              variant="outline"
              className="gap-1.5 text-[11px]"
              data-testid="badge-greenhouses-coverage"
            >
            ...
            <Badge
              variant="outline"
              className="gap-1.5 text-[11px]"
              data-testid="badge-observations-coverage"
            >
      ...
      <div
        ...
        style={{ gridTemplateColumns: selected ? '280px 1fr 320px' : '280px 1fr' }}
      >
        <Card
