...
import { queueCreateHarvest, queueUpdateHarvestLot, queueDeleteHarvestLot } from "@/hooks/data/mutations";
  ...
  const submit = form.handleSubmit(async (values) => {
    ...
    try {
      await queueCreateHarvest({
        ...
        status: "pending-sync",
        campaignId: values.campaignId && values.campaignId !== NONE_CAMPAIGN ? values.campaignId : undefined,
        unitPrice:
          typeof values.unitPrice === "number" && !Number.isNaN(values.unitPrice)
            ? values.unitPrice
            : undefined,
        currency:
          (typeof values.unitPrice === "number" || typeof values.costAllocated === "number") && values.currency
            ? values.currency
            : undefined,
        costAllocated:
    ...
    <Form {...form}>
      <form className="space-y-4" onSubmit={submit}>
        <FormField control={form.control} name="code" render={({ field }) => (
          <FormItem><FormLabel>Código de lote</FormLabel><FormControl><Input {...field} data-testid="input-lot-code" /></FormControl><FormMessage /></FormItem>
        )} />
        <FormField control={form.control} name="originRef" render={({ field }) => (
          <FormItem><FormLabel>Origen</FormLabel>
            <Select value={field.value} onValueChange={field.onChange}>
              <FormControl><SelectTrigger data-testid="select-lot-origin"><SelectValue placeholder="Bloque o invernadero…" /></SelectTrigger></FormControl>
              <SelectContent>
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField control={form.control} name="crop" render={({ field }) => (
            <FormItem><FormLabel>Cultivo</FormLabel><FormControl><Input placeholder="Tomate" {...field} data-testid="input-lot-crop" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="variety" render={({ field }) => (
            <FormItem><FormLabel>Variedad</FormLabel><FormControl><Input placeholder="Río Grande" {...field} data-testid="input-lot-variety" /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <div className="grid grid-cols-3 gap-3">
          <FormField control={form.control} name="date" render={({ field }) => (
            <FormItem><FormLabel>Fecha</FormLabel><FormControl><Input type="date" {...field} data-testid="input-lot-date" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="quantity" render={({ field }) => (
            <FormItem><FormLabel>Cantidad</FormLabel><FormControl><Input type="number" step="0.01" {...field} data-testid="input-lot-qty" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="unit" render={({ field }) => (
            <FormItem><FormLabel>Unidad</FormLabel><FormControl><Input placeholder="kg" {...field} data-testid="input-lot-unit" /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <FormField control={form.control} name="destination" render={({ field }) => (
          <FormItem><FormLabel>Destino (opcional)</FormLabel><FormControl><Input placeholder="Mercado / Cliente" {...field} data-testid="input-lot-dest" /></FormControl></FormItem>
        )} />
        <FormField control={form.control} name="campaignId" render={({ field }) => (
          <FormItem><FormLabel>Campaña (opcional)</FormLabel>
            <Select value={field.value ?? NONE_CAMPAIGN} onValueChange={field.onChange}>
              <FormControl><SelectTrigger data-testid="select-lot-campaign"><SelectValue placeholder="Sin campaña" /></SelectTrigger></FormControl>
              <SelectContent>
                <SelectItem value={NONE_CAMPAIGN}>Sin campaña</SelectItem>
                {matchingCampaigns.map((c) => (
                  <SelectItem key={c.id} value={c.id}>{c.crop} · {c.variety} ({c.scopeName})</SelectItem>
                ))}
              </SelectContent>
            </Select><FormMessage />
          </FormItem>
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField control={form.control} name="unitPrice" render={({ field }) => (
            <FormItem><FormLabel>Precio unitario (opcional)</FormLabel><FormControl><Input type="number" step="0.01" placeholder="Ej: 15.00" {...field} value={field.value ?? ""} data-testid="input-lot-price" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="currency" render={({ field }) => (
            <FormItem><FormLabel>Moneda</FormLabel><FormControl><Input maxLength={8} placeholder="BOB" {...field} value={field.value ?? ""} data-testid="input-lot-currency" /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <FormField control={form.control} name="costAllocated" render={({ field }) => (
          <FormItem>
            <FormLabel>Costo asignado al lote (opcional)</FormLabel>
            <FormControl>
              <Input type="number" step="0.01" placeholder="Ej: 120.00" {...field} value={field.value ?? ""} data-testid="input-lot-cost" />
            </FormControl>
            <p className="text-[11px] text-muted-foreground">
              Úsalo si ya tienes un costo estimado de insumos/jornales para este lote. Sin este dato, el margen no se calcula.
            </p>
            <FormMessage />
          </FormItem>
        )} />
        <Button type="submit" className="w-full" data-testid="button-submit-lot">Registrar lote</Button>
      </form>
    </Form>
  );
}
...
function EditHarvestForm({ lot, onDone }: { lot: HarvestLot; onDone: () => void }) {
  ...
  return (
    <Form {...form}>
      <form className="space-y-4" onSubmit={submit}>
        <FormField control={form.control} name="code" render={({ field }) => (
          <FormItem><FormLabel>Código de lote</FormLabel><FormControl><Input {...field} data-testid="input-lot-edit-code" /></FormControl><FormMessage /></FormItem>
        )} />
        <div className="grid grid-cols-3 gap-3">
          <FormField control={form.control} name="date" render={({ field }) => (
            <FormItem><FormLabel>Fecha</FormLabel><FormControl><Input type="date" {...field} data-testid="input-lot-edit-date" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="quantity" render={({ field }) => (
            <FormItem><FormLabel>Cantidad</FormLabel><FormControl><Input type="number" step="0.01" {...field} data-testid="input-lot-edit-qty" /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField control={form.control} name="unit" render={({ field }) => (
            <FormItem><FormLabel>Unidad</FormLabel><FormControl><Input {...field} data-testid="input-lot-edit-unit" /></FormControl><FormMessage /></FormItem>
          )} />
        </div>
        <FormField control={form.control} name="destination" render={({ field }) => (
          <FormItem><FormLabel>Destino</FormLabel><FormControl><Input {...field} data-testid="input-lot-edit-dest" /></FormControl></FormItem>
        )} />
        <FormField control={form.control} name="status" render={({ field }) => (
          <FormItem><FormLabel>Estado</FormLabel>
            <Select value={field.value} onValueChange={field.onChange}>
              <FormControl><SelectTrigger data-testid="select-lot-edit-status"><SelectValue /></SelectTrigger></FormControl>
              <SelectContent>
        ...
        <Button type="submit" className="w-full" data-testid="button-submit-lot-edit">Guardar cambios</Button>
      </form>
    </Form>
  );
}
...
export default function HarvestPage() {
  ...
  const crops = useMemo(() => Array.from(new Set(harvestLots.map((l) => l.crop))).sort(), [harvestLots]);
  const filtered = useMemo(() => {
    const ql = q.toLowerCase();
    return [...harvestLots]
      .filter((l) => {
        const matchesQ = !q || [l.code, l.crop, l.variety, l.origin, l.destination ?? ""].join(" ").toLowerCase().includes(ql);
        ...
        actions={
          !isMobile && canWrite && (
            <QuickCaptureDrawer
              ...
              trigger={<Button size="sm" data-testid="button-new-lot"><Plus className="h-4 w-4" /> Registrar lote</Button>}
      ...
      {harvestLots.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            <MetricCard label="Lotes registrados" value={harvestLots.length} icon={PackageCheck} tone="primary" />
            <MetricCard label="Total cosechado (kg)" value={totalKg.toFixed(1)} hint="Solo unidades en kg" />
            <MetricCard
          ...
          <FilterBar
            ...
            right={
              <Select value={cropFilter} onValueChange={setCropFilter}>
                <SelectTrigger className="h-9 w-44" data-testid="select-lot-crop-filter"><SelectValue placeholder="Todos los cultivos" /></SelectTrigger>
                <SelectContent>
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {canWrite && <RowActionsMenu testId={`lot-${l.id}`} onEdit={() => setEditLot(l)} onDelete={() => setDeleteId(l.id)} />}
                      </div>
                    </div>
                    <p className="mt-3 text-base font-semibold">{l.crop}</p>
                    <p className="text-xs text-muted-foreground">{l.variety} · origen {l.origin}</p>
                    <div className="mt-4 grid grid-cols-2 gap-2 border-t border-border/60 pt-3 text-xs">
                      <div>
                        ...
                        <p className="mt-0.5 text-sm font-semibold tabular">{l.quantity} <span className="text-muted-foreground font-normal">{l.unit}</span></p>
                      </div>
                      <div>
                    ...
                    {(typeof l.unitPrice === "number" || typeof l.costAllocated === "number") && (() => {
                      ...
                      return (
                        <div className="mt-3 space-y-2 border-t border-border/60 pt-3" data-testid={`margin-${l.id}`}>
                          <div className="grid grid-cols-3 gap-2 text-xs">
                              ...
                              <p className="mt-0.5 text-sm font-semibold tabular">
                                {hasPrice ? (
                                  <>
                                    ...
                                    <span className="text-muted-foreground font-normal">{l.currency ?? ""}</span>
                                  </>
                                ) : (
                            ...
                            <div>
                              ...
                              {canMargin ? (() => {
                                ...
                                return (
                                  <p className={`mt-0.5 text-sm font-semibold tabular ${positive ? "text-status-ok" : "text-status-critical"}`}>
                                    {margin.toFixed(2)} <span className="text-muted-foreground font-normal">{l.currency ?? ""}</span>
                                  </p>
                                );
                              })() : (
                                <p className="mt-0.5 text-sm font-semibold tabular text-muted-foreground">—</p>
                              )}
                            </div>
                          </div>
                          {hasPrice && !hasCost && (
                            <p className="text-[11px] text-muted-foreground" data-testid={`margin-warning-${l.id}`}>
