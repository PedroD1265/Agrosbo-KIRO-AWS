  ...
  const commitMut = useMutation({
    ...
    onSuccess: (result) => {
      if (result.committed) {
        ...
        toast.error("No se pudo importar", { description: "Corrige los errores antes de continuar." });
      }
    },
    onError: (e: Error) => {
    ...
    <Card className="border-border/60 shadow-card">
      <CardHeader className="border-b border-border/40 pb-4">
        ...
        <p className="text-xs text-muted-foreground">
          Carga masiva con validación previa. La importación solo se ejecuta si todas las filas son válidas.
        </p>
            ...
            <div key={s.n} className="flex items-center gap-1.5">
              <span
                className={cn(
                  ...
                  stepNumber >= s.n ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground",
                )}
              >
                {s.n}
              </span>
              <span className={cn(stepNumber >= s.n ? "text-foreground" : "text-muted-foreground")}>{s.label}</span>
      ...
      <CardContent className="space-y-4 p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {fileName && (
            <Button variant="ghost" size="sm" onClick={handleReset} data-testid="button-reset-import">
              Limpiar
            </Button>
          )}
        </div>
          ...
          <div className="space-y-3" data-testid="section-import-preview">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <div className="rounded-md border border-status-ok/20 bg-status-ok-soft px-3 py-2">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-status-ok">Válidas</p>
                <p className="mt-0.5 text-lg font-semibold tabular text-status-ok" data-testid="text-valid-rows">
              ...
              >
                <p
                  className={cn(
                    ...
                    previewResult.columnErrors.length > 0 ? "text-status-critical" : "text-muted-foreground",
                  )}
                >
                  Columnas
                </p>
                <p
                  className={cn(
                    ...
                    previewResult.columnErrors.length > 0 ? "text-status-critical" : "text-foreground",
                  )}
                  data-testid="text-column-error-count"
                >
                  {previewResult.columnErrors.length === 0 ? "OK" : `−${previewResult.columnErrors.length}`}
                </p>
              </div>
              <div
                className={cn(
              ...
              >
                <p
                  className={cn(
                    "text-[10px] font-semibold uppercase tracking-wide",
                    previewResult.errors.length > 0 ? "text-status-critical" : "text-muted-foreground",
                  )}
                >
                  Filas con error
                </p>
            ...
            {previewResult.columnErrors.length > 0 && (
              <div className="rounded-md border border-status-critical/30 bg-status-critical-soft p-3" data-testid="section-column-errors">
                ...
                <div className="space-y-1">
                  {previewResult.columnErrors.map((err, i) => (
                    <div key={i} className="text-xs text-status-critical" data-testid={`row-column-error-${i}`}>
            ...
            {previewResult.columnErrors.length === 0 && previewResult.preview.length > 0 && (
              <div className="overflow-x-auto rounded-md border border-border" data-testid="table-import-preview">
                  ...
                  <thead className="border-b bg-muted/40 text-[10px] uppercase tracking-wide text-muted-foreground">
                    <tr>
                      ...
                      {Object.keys(previewResult.preview[0]?.data ?? {}).map((col) => (
                        <th key={col} className="px-2 py-2 text-left max-w-[120px] truncate">{col}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {previewResult.preview.map((prow) => (
                      ...
                      >
                        ...
                        {Object.values(prow.data).map((val, ci) => (
                          <td key={ci} className="px-2 py-1.5 max-w-[120px] truncate text-foreground">
                            {String(val ?? "")}
                          </td>
                        ))}
                      </tr>
            ...
            {previewResult.columnErrors.length === 0 && previewResult.errors.length > 0 && (
              <div className="rounded-md border border-status-critical/30 bg-status-critical-soft p-3" data-testid="section-import-errors">
                <p className="mb-2 text-xs font-semibold text-status-critical">
                  Errores de validación — corrija el archivo y vuelva a cargarlo:
                </p>
                <div className="space-y-1 max-h-40 overflow-y-auto">
                  {previewResult.errors.map((err, i) => (
                    <div key={i} className="text-xs text-status-critical" data-testid={`row-import-error-${i}`}>
            ...
            >
              ...
              {commitMut.isPending ? "Importando…" : `Confirmar importación (${previewResult.validRows} filas)`}
            </Button>
          </div>
        )}

        <div className="rounded-md border border-border/50 bg-muted/30 p-3">
          <p className="text-xs font-semibold text-foreground mb-1.5">Columnas requeridas por dataset</p>
          <div className="space-y-1 text-[11px] text-muted-foreground font-mono">
            <div><span className="font-semibold text-foreground font-sans">Bloques:</span> name, farm, areaHa, crop, stage, lastIrrigation, status</div>
            <div><span className="font-semibold text-foreground font-sans">Invernaderos:</span> name, areaM2, crop, stage, status</div>
            <div><span className="font-semibold text-foreground font-sans">Inventario:</span> name, category, unit, stock, min, lastMovement</div>
            <div><span className="font-semibold text-foreground font-sans">Tareas:</span> title, scopeType, scopeId, assignee, dueDate, priority</div>
          </div>
