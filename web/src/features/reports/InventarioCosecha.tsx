    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <MetricCard
          ...
          icon={AlertTriangle}
          tone={d.lowStock.length > 0 ? 'critical' : 'ok'}
          hint={d.lowStock.length === 0 ? 'Niveles correctos' : 'Reponer pronto'}
        />
        <MetricCard
          label="Lotes cosecha"
          value={d.lots.length}
          icon={Sprout}
          tone={d.lots.length > 0 ? 'primary' : 'default'}
        />
            ...
            actions={[
              {
                ...
                testId: 'button-export-inventario',
                count: d.lowStock.length,
              },
            ]}
          />
      ...
      <ReportTable<InventoryItem>
        ...
        columns={[
          ...
          {
            header: 'Categoría',
            accessor: (i) => <span className="text-muted-foreground">{i.category}</span>,
          },
          {
            header: 'Stock',
            accessor: (i) => (
              <span className="tabular font-semibold text-status-critical">
                {i.stock} <span className="font-normal text-muted-foreground">{i.unit}</span>
              </span>
            ),
            align: 'right',
          },
          ...
          },
          {
            header: 'Déficit',
            accessor: (i) => (
              <StatusBadge status="critical" label={`-${(i.min - i.stock).toFixed(1)} ${i.unit}`} />
            ),
          },
          {
            header: 'Último mov.',
            accessor: (i) => (
              <span className="tabular text-muted-foreground">{i.lastMovement}</span>
            ),
          },
        ]}
      ...
      <ReportTable<HarvestLot>
        ...
        columns={[
          {
            header: 'Código',
            accessor: (h) => <span className="font-mono text-xs font-semibold">{h.code}</span>,
          },
          {
            header: 'Origen',
            accessor: (h) => <span className="text-muted-foreground">{h.origin}</span>,
          },
          ...
          { header: 'Fecha', accessor: (h) => <span className="tabular">{h.date}</span> },
          {
            ...
            accessor: (h) => (
              <span className="tabular font-medium">
                {h.quantity} <span className="font-normal text-muted-foreground">{h.unit}</span>
              </span>
            ),
            align: 'right',
          },
          {
            header: 'Destino',
            accessor: (h) => h.destination ?? <span className="text-muted-foreground">—</span>,
          },
        ...
        <>
          ...
          <ReportTable
            ...
            columns={[
              ...
              {
                header: 'Variedad',
                accessor: (r) => <span className="text-muted-foreground">{r.variety}</span>,
              },
              {
                header: 'Lotes',
