...
export function useResumenOperativo() {
  ...
  return useMemo(() => {
    ...
    const scheduledIrrigation = data.irrigationEvents.filter((e) => e.status === "scheduled").length;
...
export function useObservationsReport(filters: ReportFilters) {
  ...
  return useMemo(() => {
    let observations = data.observations.filter(
      (o) => o.createdAt.slice(0, 10) >= filters.dateFrom && o.createdAt.slice(0, 10) <= filters.dateTo,
    );
    if (filters.scopeType !== "all") observations = observations.filter((o) => o.scopeType === filters.scopeType);
    if (filters.scopeId !== "all") observations = observations.filter((o) => o.scopeId === filters.scopeId);
...
export function useIrrigationReport(filters: ReportFilters) {
  ...
  return useMemo(() => {
    ...
    const byScope: Record<string, { name: string; done: number; scheduled: number; skipped: number; total: number }> = {};
...
export function useTaskReport(filters: ReportFilters) {
  ...
  return useMemo(() => {
    ...
    return { tasks, overdue, recentDone, byAssigneeList, byPriority, assignees, isLoading: data.isLoading };
  }, [data, filters]);
}
...
export function useInventoryHarvestReport(filters: ReportFilters) {
  ...
  return useMemo(() => {
    ...
    const byCrop: Record<string, { crop: string; variety: string; total: number; unit: string; count: number }> = {};
    for (const h of lots) {
      const key = `${h.crop}|${h.variety}|${h.unit}`;
      if (!byCrop[key]) byCrop[key] = { crop: h.crop, variety: h.variety, total: 0, unit: h.unit, count: 0 };
