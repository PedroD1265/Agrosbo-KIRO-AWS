    ...
    exportCsv(
      ...
      [
        ...
        { header: 'Etapa', accessor: (r) => r.greenhouse.stage },
        {
          ...
          accessor: (r) =>
            r.activeCampaign ? `${r.activeCampaign.crop} (${r.activeCampaign.variety})` : '',
        },
    ...
    <div className="space-y-4">
            ...
            actions={[
              {
                ...
                testId: 'button-export-invernaderos',
                count: rows.length,
              },
            ]}
          />
      ...
      <ReportTable<Row>
        ...
        columns={[
          {
            header: 'Invernadero',
            accessor: (r) => <span className="font-medium">{r.greenhouse.name}</span>,
          },
          {
            header: 'Campaña activa',
            accessor: (r) =>
              r.activeCampaign ? (
                `${r.activeCampaign.crop} — ${r.activeCampaign.variety}`
              ) : (
          ...
          },
          {
            ...
            accessor: (r) => (
              <span
                className={`tabular ${r.openTasks > 0 ? 'font-semibold text-status-warn' : 'text-muted-foreground'}`}
              >
                {r.openTasks}
              </span>
            ),
          ...
          },
          {
            header: 'Último riego',
            accessor: (r) => (
              <span className="tabular text-muted-foreground">{r.lastIrrigationDate}</span>
            ),
          },
          {
            header: 'Observaciones',
            accessor: (r) => <span className="tabular">{r.observations}</span>,
            align: 'right',
          },
          {
            header: 'Lotes cosecha',
            accessor: (r) => <span className="tabular">{r.harvestLots}</span>,
            align: 'right',
          },
          {
            header: 'Temp / Hum',
            accessor: (r) => (
              <span className="tabular text-muted-foreground">
                {r.greenhouse.tempC != null
                  ? `${r.greenhouse.tempC}°C / ${r.greenhouse.humidity ?? '—'}%`
                  : '—'}
              </span>
            ),
          },
