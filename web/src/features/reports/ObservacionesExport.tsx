    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <MetricCard
          ...
          icon={Eye}
          tone={observations.length > 0 ? 'primary' : 'default'}
        />
        <MetricCard
          label="Incidentes"
          value={incidents}
          icon={AlertTriangle}
          tone={incidents > 0 ? 'warn' : 'default'}
        />
        <MetricCard
          label="Plagas / enfermedades"
          value={pests}
          icon={Bug}
          tone={pests > 0 ? 'critical' : 'default'}
        />
      </div>
            ...
            actions={[
              {
                ...
                testId: 'button-export-observaciones',
                count: observations.length,
              },
            ]}
          />
        ...
        columns={[
          {
            header: 'Fecha',
            accessor: (o) => (
              <span className="tabular text-muted-foreground">{o.createdAt.slice(0, 10)}</span>
            ),
          },
          {
            header: 'Ubicación',
            accessor: (o) => <span className="font-medium">{o.scopeName}</span>,
          },
          { header: 'Autor', accessor: (o) => o.author },
          {
            header: 'Tipo',
          ...
          },
          {
            header: 'Texto',
            accessor: (o) => (
              <span className="line-clamp-2 max-w-xs text-muted-foreground">{o.text}</span>
            ),
          },
        ]}
