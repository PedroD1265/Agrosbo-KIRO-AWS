...
export function ResumenOperativo() {
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                ...
                {d.lowStock > 0 &&
                  `${d.lowStock} insumo${d.lowStock > 1 ? 's' : ''} con stock bajo`}
                {d.lowStock > 0 && d.scheduledIrrigation > 0 && ' · '}
                {d.scheduledIrrigation > 0 &&
                  `${d.scheduledIrrigation} riego${d.scheduledIrrigation > 1 ? 's' : ''} programado${d.scheduledIrrigation > 1 ? 's' : ''}`}
      ...
      <ReportSectionHeader
        title="Tareas"
        description="Carga de trabajo y vencimientos"
        icon={CheckSquare}
        tone="primary"
      />
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
      ...
      <ReportSectionHeader
        title="Riego"
        description="Programación y cumplimiento"
        icon={Droplets}
        tone="primary"
      />
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        <MetricCard
          label="Programados"
          value={d.scheduledIrrigation}
          icon={Droplets}
          tone={d.scheduledIrrigation > 0 ? 'warn' : 'default'}
        />
        <MetricCard label="Completados" value={d.doneIrrigation} icon={Droplets} tone="ok" />
      </div>

      <ReportSectionHeader
        title="Bitácora y producción"
        description="Observaciones, stock y cosecha"
        icon={Eye}
        tone="primary"
      />
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
