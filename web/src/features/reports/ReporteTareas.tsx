  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
        <MetricCard label="Alta prioridad pend." value={d.byPriority.high} icon={AlertTriangle} tone={d.byPriority.high > 0 ? "critical" : "default"} />
        <MetricCard label="Media prioridad pend." value={d.byPriority.med} icon={BarChart2} tone={d.byPriority.med > 0 ? "warn" : "default"} />
        <MetricCard label="Vencidas" value={d.overdue.length} icon={AlertTriangle} tone={d.overdue.length > 0 ? "critical" : "default"} hint={d.overdue.length === 0 ? "Sin vencidas" : "Atender pronto"} />
      </div>
      ...
      <ReportSectionHeader
        ...
        actions={<ExportBar actions={[{ label: "Tareas", onExport: handleExport, testId: "button-export-tareas", count: d.tasks.length }]} />}
      />
      ...
      <ReportTable<Task>
        ...
        columns={[
          ...
          { header: "Ubicación", accessor: (t) => <span className="text-muted-foreground">{t.scopeName}</span> },
          { header: "Responsable", accessor: (t) => t.assignee },
          { header: "Vence", accessor: (t) => <span className="tabular text-status-critical">{t.dueDate}</span> },
          { header: "Prioridad", accessor: (t) => <StatusBadge status={t.priority === "high" ? "critical" : t.priority === "med" ? "warn" : "idle"} label={priorityLabel[t.priority]} /> },
        ]}
      ...
      <ReportTable
        ...
        columns={[
          ...
          { header: "Tareas pendientes / en curso", accessor: (r) => <span className="tabular">{r.count}</span>, align: "right" },
        ]}
