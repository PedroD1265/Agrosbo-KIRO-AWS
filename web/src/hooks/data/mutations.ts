...
function lookupScopeName(scopeType: ScopeType, scopeId: string): string {
  if (scopeType === 'block') {
    const blocks = queryClient.getQueryData<Block[]>(['/api/blocks']) ?? [];
    return blocks.find((b) => b.id === scopeId)?.name ?? scopeId;
  }
  const greenhouses = queryClient.getQueryData<Greenhouse[]>(['/api/greenhouses']) ?? [];
...
export async function queueUpdateTaskStatus(id: string, status: TaskStatus) {
  patchList<Task>(['/api/tasks'], (old) => old.map((t) => (t.id === id ? { ...t, status } : t)));
  await enqueue({
...
export async function queueCreateIrrigation(input: CreateIrrigationInput) {
  ...
  patchList<IrrigationEvent>(['/api/irrigation-events'], (old) => [...old, optimistic]);
  await enqueue({
    clientId,
...
export async function queueAdjustInventory(input: AdjustInventoryInput) {
  ...
  await enqueue({
    ...
    invalidateKeys: [
      ['/api/inventory'],
      ['/api/inventory', input.id, 'movements'],
      ['/api/alerts'],
    ],
  });
  triggerSync();
...
export async function queueUpdateBlockGeometry(id: string, patch: BlockGeometryPatch) {
    ...
    old.map((b) =>
      b.id === id
        ? {
            ...
            ...(patch.centroidLat !== undefined
              ? { centroidLat: patch.centroidLat ?? undefined }
              : {}),
            ...(patch.centroidLng !== undefined
              ? { centroidLng: patch.centroidLng ?? undefined }
              : {}),
          }
        : b,
    ),
...
export async function queueUpdateGreenhouseLocation(id: string, patch: GreenhouseLocationPatch) {
...
export async function queueUpdateObservationLocation(id: string, patch: ObservationLocationPatch) {
  patchList<Observation>(['/api/observations'], (old) =>
    old.map((o) =>
      o.id === id ? { ...o, lat: patch.lat ?? undefined, lng: patch.lng ?? undefined } : o,
    ),
  );
  await enqueue({
...
export async function queueImportBlockBoundaries(fc: GeoJsonFeatureCollection) {
  await enqueue({
...
export async function queueDeleteGreenhouse(id: string) {
  patchList<Greenhouse>(['/api/greenhouses'], (old) => old.filter((g) => g.id !== id));
  await enqueue({
...
export async function queueUpdateCampaign(id: string, patch: UpdateCampaign) {
  patchList<Campaign>(['/api/campaigns'], (old) =>
    old.map((c) => {
      ...
      const next = { ...c, ...patch };
      if (
        (patch.scopeType && patch.scopeType !== c.scopeType) ||
...
export async function queueDeleteIrrigation(id: string) {
  patchList<IrrigationEvent>(['/api/irrigation-events'], (old) => old.filter((i) => i.id !== id));
  await enqueue({
...
export async function queueDeleteObservation(id: string) {
  patchList<Observation>(['/api/observations'], (old) => old.filter((o) => o.id !== id));
  await enqueue({
...
export async function queueDeleteInventoryItem(id: string) {
  patchList<InventoryItem>(['/api/inventory'], (old) => old.filter((i) => i.id !== id));
  await enqueue({
...
export async function queueDeleteHarvestLot(id: string) {
  patchList<HarvestLot>(['/api/harvest-lots'], (old) => old.filter((h) => h.id !== id));
  await enqueue({
...
export async function queueCreateApplication(input: CreateApplicationInput) {
  ...
  await enqueue({
    ...
    body: input,
    invalidateKeys: [['/api/applications'], ['/api/inventory'], ['/api/alerts']],
  });
  triggerSync();
}
