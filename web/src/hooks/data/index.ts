...
export function useIrrigationAdvice(scope?: { scopeType: "block" | "greenhouse"; scopeId: string }) {
  const url = scope
...
import type {
  Attachment, Expense, LaborCost, User,
...
export function useExpenses(filter?: { campaignId?: string; from?: string; to?: string; category?: string }) {
  const params = new URLSearchParams();
