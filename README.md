...
13. Consulta al Asistente AGROSBO (texto/voz). 14. Agente lee datos vía herramientas. 15. Navega visiblemente. 16. Prepara borrador. 17. Usuario confirma → cola offline → idempotencia. 18. SES acepta solicitud y devuelve message ID → sent (no implica entrega). 19. Colaborador accede al enlace → responded. 20. Foto agrícola → evaluación visual preliminar (Bedrock). 21. IrrigationDelayScenario → LLM explica.
