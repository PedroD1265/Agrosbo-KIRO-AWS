...
export async function applicationsCSV(
  filter: { from?: string; to?: string; campaignId?: string } = {},
): Promise<string> {
  const conds = [];
...
export async function carenciaActivaCSV(
...
export async function laborCSV(
  filter: { from?: string; to?: string; campaignId?: string } = {},
): Promise<string> {
  const conds = [];
...
export async function apiariesCSV(): Promise<string> {
  ...
  return rowsToCSV(
    ['id', 'name', 'location', 'lat', 'lng', 'status', 'notes', 'createdAt'],
    rows.map((r: any) => ({ ...r })),
  );
}
