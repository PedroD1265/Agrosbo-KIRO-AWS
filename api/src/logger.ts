...
function emit(level: LogLevel, scope: string, message: string, fields?: Record<string, unknown>) {
  ...
  const tail = enriched && Object.keys(enriched).length > 0 ? ' ' + JSON.stringify(enriched) : '';
...
export function requestLogger() {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on('finish', () => {
      ...
      const rawQuery = fullPath.includes('?') ? fullPath.slice(fullPath.indexOf('?') + 1) : '';
      ...
      if (typeof idemKey === 'string') {
        ...
        fields.idempotencyKeyHash = createHash('sha256').update(idemKey).digest('hex').slice(0, 12);
      }
