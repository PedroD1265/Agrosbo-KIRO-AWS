...
import {
  listApiaries, createApiary,
  listHives, createHive,
  listInspections, createInspection, HiveInventoryItemNotFoundError,
  listHoneyHarvests, createHoneyHarvest,
} from "./beekeeping.js";
...
import { listAttachments, createAttachment, deleteAttachment, AttachmentValidationError } from "./attachments.js";
import { listExpenses, createExpense, deleteExpense, listLaborCosts, createLaborCost, costsForCampaign, costsForScope } from "./expenses.js";
import { listUsers, createUser, getUserByLogin, verifyPassword } from "./users.js";
import { requireAuth, requireRole, setSessionCookie, clearSessionCookie, revokeToken, COOKIE_NAME } from "./auth.js";
import { env } from "./env.js";
import {
  applicationsCSV, carenciaActivaCSV, expensesCSV, laborCSV,
  apiariesCSV, hivesCSV, inspectionsCSV, honeyHarvestsCSV,
} from "./reports.js";
...
export function registerRoutes(router: Router) {
  ...
  router.get(
    ...
    asyncHandler(async (_req, res) =>
      res.json(await storage.listIrrigationEvents()),
    ),
  );
  router.post(
    "/irrigation-events",
  ...
  router.get(
    "/alerts",
    asyncHandler(async (_req, res) => {
      const [blocks, greenhouses, inventory, irrigation, observations, tasks, applications, hives] = await Promise.all([
        storage.listBlocks(),
      ...
      res.json(
        deriveAlerts({ blocks, greenhouses, inventory, irrigation, observations, tasks, applications, hives }),
      );
    }),
  );
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const advices = await Promise.all(targets.map(async ({ type, entity }) => {
        ...
        if (typeof lat === "number" && typeof lng === "number") {
          try { forecast = await getForecast(lat, lng); } catch { forecast = null; }
        }
        return buildIrrigationAdvice({
          scope: entity, scopeType: type, irrigationEvents: irrigation,
          campaign: camp, forecast,
        });
      }));
      res.json(advices);
    }),
   ...
   * Beekeeping
   * ============================================================== */
  router.get("/apiaries", asyncHandler(async (_req, res) => res.json(await listApiaries())));
  router.post("/apiaries", requireRole("admin", "tecnico", "encargado"), idempotent(async (req, res) => {
    const data = insertApiarySchema.parse(req.body);
    res.status(201).json(await createApiary(data));
  }));
  router.get("/hives", asyncHandler(async (_req, res) => res.json(await listHives())));
  router.post("/hives", requireRole("admin", "tecnico", "encargado"), idempotent(async (req, res) => {
    const data = insertHiveSchema.parse(req.body);
    res.status(201).json(await createHive(data));
  }));
  router.get("/hive-inspections", asyncHandler(async (req, res) => {
    const hiveId = typeof req.query.hiveId === "string" ? req.query.hiveId : undefined;
    res.json(await listInspections(hiveId));
  }));
  router.post("/hive-inspections", requireRole("admin", "tecnico", "encargado"), idempotent(async (req, res) => {
  ...
  router.get("/honey-harvests", asyncHandler(async (_req, res) => res.json(await listHoneyHarvests())));
  router.post("/honey-harvests", requireRole("admin", "tecnico", "encargado"), idempotent(async (req, res) => {
  ...
  router.post("/attachments", idempotent(async (req, res) => {
  ...
  router.post("/expenses", requireRole("admin", "tecnico", "encargado", "finanzas"), idempotent(async (req, res) => {
    const data = insertExpenseSchema.parse(req.body);
    res.status(201).json(await createExpense(data));
  }));
  router.delete("/expenses/:id", requireRole("admin", "tecnico", "encargado", "finanzas"), idempotent(async (req, res) => {
  ...
  router.get("/labor-costs", asyncHandler(async (req, res) => {
    const q = z.object({
      from: z.string().optional(),
      to: z.string().optional(),
      campaignId: z.string().optional(),
  ...
  router.post("/labor-costs", requireRole("admin", "tecnico", "encargado", "finanzas"), idempotent(async (req, res) => {
    const data = insertLaborCostSchema.parse(req.body);
    res.status(201).json(await createLaborCost(data));
  }));
  router.get("/campaigns/:id/costs", asyncHandler(async (req, res) => {
    res.json(await costsForCampaign((req.params.id as string)));
  }));
  router.get("/scope-costs", asyncHandler(async (req, res) => {
    const q = z.object({
   ...
   * ============================================================== */
  router.get("/users", requireRole("admin"), asyncHandler(async (_req, res) => res.json(await listUsers())));
  router.post("/users", requireRole("admin"), idempotent(async (req, res) => {
  ...
  router.get("/reports/applications.csv", asyncHandler(async (req, res) => {
    const q = z.object({ from: z.string().optional(), to: z.string().optional(), campaignId: z.string().optional() }).parse(req.query);
    sendCsv(res, "applications", await applicationsCSV(q));
  }));
  router.get("/reports/carencia.csv", asyncHandler(async (_req, res) => {
    sendCsv(res, "carencia", await carenciaActivaCSV());
  }));
  router.get("/reports/expenses.csv", asyncHandler(async (req, res) => {
    const q = z.object({ from: z.string().optional(), to: z.string().optional(), campaignId: z.string().optional() }).parse(req.query);
    sendCsv(res, "expenses", await expensesCSV(q));
  }));
  router.get("/reports/labor.csv", asyncHandler(async (req, res) => {
    const q = z.object({ from: z.string().optional(), to: z.string().optional(), campaignId: z.string().optional() }).parse(req.query);
    sendCsv(res, "labor", await laborCSV(q));
  }));
  router.get("/reports/apiaries.csv", asyncHandler(async (_req, res) => sendCsv(res, "apiaries", await apiariesCSV())));
  router.get("/reports/hives.csv", asyncHandler(async (_req, res) => sendCsv(res, "hives", await hivesCSV())));
  router.get("/reports/inspections.csv", asyncHandler(async (_req, res) => sendCsv(res, "inspections", await inspectionsCSV())));
  router.get("/reports/honey-harvests.csv", asyncHandler(async (_req, res) => sendCsv(res, "honey-harvests", await honeyHarvestsCSV())));
  router.get("/reports/honey.csv", asyncHandler(async (_req, res) => sendCsv(res, "honey-harvests", await honeyHarvestsCSV())));
  ...
  router.get("/health", asyncHandler(async (_req, res) => {
    let dbOk = false;
    try { await storage.getSettings(); dbOk = true; } catch { dbOk = false; }
    res.json({
      ok: dbOk,
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!updated) {
        ...
        return res.status(400).json({ error: "No se puede habilitar: el adaptador requiere secretos no configurados." });
      }
      res.json(updated);
    }),
  );
  ...
  router.use(
    (err: unknown, _req: Request, res: Response, _next: NextFunction) => {
      if (err instanceof ZodError) {
        return res
          .status(400)
          .json({ error: "Datos inválidos", issues: err.issues });
      }
